...
def resol(a,b,c,A): #a,b,c sont des fonctions respectivement second/premier ordre 
#First order differential equation with or without second member
	...
	if a == 0: 
		res = k*exp(-integrate(c/b,x)) + k*exp(-integrate(c/b,x))*integrate(A/(b*exp(-integrate(c/b,x))),x) #solution générale variation de la constante
	...
	if d > 0 :
		...
		if A == k*exp(k_1*x): #Condition solution particulière
			...
			deg_sol = degree(A) 
			res += poly(deg_sol)+"*"+str(exp(k_1*x)*(L*sin(w*x) + M*cos(w*x)))
		else:
			if c != 0:
				res += poly(deg_sol)
			elif c == 0 and b != 0:
